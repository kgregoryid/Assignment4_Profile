...
//Rare exceptions to indicate a bug in the _profile code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_profile = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_profile]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.profile.module"
